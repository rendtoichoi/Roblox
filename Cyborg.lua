...
local function TweenChest(chest)
    if not chest or not chest:IsA("BasePart") or not chest.Parent then
        return false
    end
    ...
    repeat
        ...
        if CheckTool("Fist of Darkness") or IsDied(Character) then
            break
        end
    ...
    until touched or not chest.CanTouch or tick() - startTick > timeout

    pcall(function() chestTween:Cancel() end)
...
local function TweenTouchPart(part, text, speedSetting, radiusSetting, stopCondition)
    ...
    local speed = tonumber(getgenv().Settings[speedSetting or "Flower Tween Speed"]) or 325
    local touchRadius = tonumber(getgenv().Settings[radiusSetting or "Flower Touch Radius"]) or 8
    ...
    local partTween = TweenService:Create(
        ghost,
        TweenInfo.new(travelTime, Enum.EasingStyle.Linear),
    ...
    pcall(function() partTween:Cancel() end)
    if hb then hb:Disconnect() end
    if ghost then ghost:Destroy() end
                            ...
                            for _, v in next, workspace.Enemies:GetChildren() do
                                if v.Name == "Order" then
                                    if v:FindFirstChildWhichIsA("Humanoid") and v.Humanoid.Health > 0 and v.HumanoidRootPart then
                                        repeat task.wait()
                                            pcall(function()
                                                local bossHumanoid = v:FindFirstChildWhichIsA("Humanoid")
                                                if bossHumanoid and bossHumanoid.MaxHealth > 0 then
                                                    SetText(
                                                        "Killing Law / Order\n" ..
                                                        "Health: " .. math.floor(bossHumanoid.Health / bossHumanoid.MaxHealth * 100) .. "%\n" ..
                                                        "Fragments: " .. tostring(LocalPlayer.Data.Fragments.Value)
                                                    )
                                                else
                                                    SetText("Killing Law / Order")
                                                end
                                            end)
                                            KillMonster(v.Name)
                            ...
                            for i, t in next, chests do local v = t.obj
                                if v:IsA("BasePart") and v.Name:find("Chest") then
                                    if v.CanTouch then
                                        repeat
                                            task.wait()
                                            SetText("Collect Chests | Tween Chest | Collected: " .. c.."/"..all .. "/"..getgenv().Settings["Max Chests"].." Chests")
                                            local touched = TweenChest(v)
